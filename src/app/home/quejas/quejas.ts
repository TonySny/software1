...
export class QuejasComponent {
  ...
  async enviarQueja() {
    ...
    try {
      ...
      if (error) {
        Swal.fire('Error', 'No se pudo enviar la queja: ' + error.message, 'error');
      } else {
        Swal.fire('Enviado', `Tu queja fue enviada correctamente. Número de radicado: ${numeroRadicado}`, 'success');
        this.limpiarFormulario();
      }
    } catch (err) {
      Swal.fire('Error', 'Error inesperado al enviar la queja', 'error');
    }
  }
